...
pub struct Session {
    ...
    pub host: Pubkey,                    // 32
    pub total_usdt: u64,                 // 8 - total bill in USDT (6 decimals)
    ...
    pub total_collected: u64,            // 8 - sum of all USDT deposits so far
    ...
    pub recipient_token_account: Pubkey, // 32 - recipient's USDT token account
    pub usdt_vault: Pubkey, // 32 - vault token account pubkey (prevents rogue vault attack)
    pub bump: u8,           // 1
}
